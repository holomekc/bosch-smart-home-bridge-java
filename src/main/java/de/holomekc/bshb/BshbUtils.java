 ...
 * @since 12.01.2020
 */
public class BshbUtils {

    /**
     * Generate a random identifier which is needed during pairing process.
     *
     * @return a random identifier which is needed during pairing process.
     */
     ...
     *
     * @return root ca certificate of Bosch Smart Home Controller.
     */
    public static String getBoschSmartHomeControllerRootCa() {
    ...
    public static CertificateDefinition generateClientCertificate() {
        ...
        try {
            ...
            final X500Name name = new X500NameBuilder().addRDN(BCStyle.CN, "bosch-smart-home-bridge").build();

            final X509v3CertificateBuilder builder = new X509v3CertificateBuilder(name, sn, from, to, name,
                    SubjectPublicKeyInfo.getInstance(keyPair.getPublic().getEncoded()));

            final AlgorithmIdentifier sigAldIdentifier =
                    new DefaultSignatureAlgorithmIdentifierFinder().find("SHA512withRSA");

            final ContentSigner contentSigner = new BcRSAContentSignerBuilder(sigAldIdentifier,
                    new DefaultDigestAlgorithmIdentifierFinder().find(sigAldIdentifier))
                    .build(PrivateKeyFactory.createKey(keyPair.getPrivate().getEncoded()));

            return new CertificateDefinition("-----BEGIN CERTIFICATE-----\n" + new String(
                    Base64.getEncoder().encode(builder.build(contentSigner).getEncoded()))
                    + "\n-----END CERTIFICATE-----", "-----BEGIN PRIVATE KEY-----\n" + new String(
                    Base64.getEncoder().encode(keyPair.getPrivate().getEncoded())) + "\n-----BEGIN PRIVATE KEY-----");
        } catch (final NoSuchAlgorithmException | IOException | OperatorCreationException e) {
